...
typedef struct sal_av_args
{
    ...
    int vpss_chn[16];         //vpss已经创建通道id
    int venc_chn[16];         //venc已经创建通道id
    int vpss_venc_chn[16][2];//vpss和venc已经创通道的绑定关系
...
static int vi_set_dev(VI_DEV ViDev, ISP_DEV IspDev, SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32 s32Ret = -1;
...
static int vi_unbind_vpss(VI_DEV ViDev, VPSS_GRP VpssGrp)
{
    HI_S32 s32Ret = -1;
    MPP_CHN_S stSrcChn;
...
static int isp_init(ISP_DEV IspDev, WDR_MODE_E  enWDRMode)
{
    HI_S32 s32Ret = -1;
    ...
    ISP_SNS_OBJ_S* pstSnsObj = g_pstSnsObj[IspDev];
    CHECK(pstSnsObj != NULL && pstSnsObj->pfnRegisterCallback != NULL, HI_FAILURE, "Error with %#x.\n", pstSnsObj);
...
static int isp_start(ISP_DEV IspDev)
{
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setstacksize(&attr, 4096 * 1024);
    if (0 != pthread_create(&g_av_args->isp_tid, &attr, isp_run, (HI_VOID*)IspDev))
    {
...
static int isp_stop(ISP_DEV IspDev)
{
...
static int vi_stop_isp(VI_DEV ViDev, ISP_DEV IspDev)
{
    VI_CHN ViChn = ViDev;
    HI_S32 s32Ret = -1;
    ...
    s32Ret = isp_stop(IspDev);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
    
    return HI_SUCCESS;
}

static int vi_start_all(VI_DEV ViDev, ISP_DEV IspDev)
{
    ...
    VI_CHN ViChn = ViDev;
    SIZE_S stTargetSize;
    memset(&stTargetSize, 0, sizeof(stTargetSize));
    RECT_S stCapRect;
    ...
    s32Ret = isp_init(IspDev, stViConfig.enWDRMode);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);

    s32Ret = isp_start(IspDev);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);

    s32Ret = vi_set_dev(ViDev, IspDev, &stViConfig);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
...
static int vpss_bind_vi(VI_DEV ViDev, VPSS_GRP VpssGrp)
{
    HI_S32 s32Ret = -1;
    MPP_CHN_S stSrcChn;
    memset(&stSrcChn, 0, sizeof(stSrcChn));
...
static int vpss_start(VI_DEV ViDev, VPSS_GRP VpssGrp)
{
    int s32Ret = 0;
...
static int venc_index_find(VENC_CHN VencChn)
{
    unsigned int i = 0;
    for (i = 0; i < sizeof(g_av_args->venc_chn)/sizeof(g_av_args->venc_chn[0]); i++)
    {
        if (g_av_args->venc_chn[i] != -1 && g_av_args->venc_chn[i] == VencChn)
        {
            return i;
        }
    }
...
static int venc_enable_vui(VENC_CHN VencChn)
{
    ...
    int idx = venc_index_find(VencChn);
    CHECK(idx >= 0, HI_FAILURE, "Error with %#x.\n", idx);
    
    sal_stream_s* stream = &g_av_args->video.stream[idx];
...
static int venc_set_chn(VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
    int s32Ret = 0;
...
static int venc_set_chn_ext(VPSS_GRP VpssGrp, VPSS_CHN VpssPhyChn, VPSS_CHN VpssExtChn)
{
    int s32Ret = 0;
...
static int venc_bind_vpss(VPSS_GRP VpssGrp, VENC_CHN VencChn, VPSS_CHN VpssChn)
{
...
static int venc_set_cfg(VPSS_GRP VpssGrp, VENC_CHN VencChn)
{
...
static int venc_one_pack(VENC_CHN VencChn, VENC_STREAM_S* pstStream, VENC_CHN_STAT_S* pstStat)
{
    HI_S32 s32Ret = HI_FAILURE;
    VENC_PACK_S pack;
    ...
    int timeout = 0; // 非阻塞
    s32Ret = HI_MPI_VENC_GetStream(VencChn, pstStream, timeout);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
    ...
    s32Ret = venc_write_cb(VencChn, pstStream->pstPack->u64PTS, frame_addr, frame_len, isKey, stream->encode_type);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
...
static int venc_multiple_pack(VENC_CHN VencChn, VENC_STREAM_S* pstStream, VENC_CHN_STAT_S* pstStat)
{
    HI_S32 s32Ret = HI_FAILURE;
    ...
    s32Ret = venc_write_cb(VencChn, pstStream->pstPack[j-1].u64PTS, buffer, buffer_offset, isKey, stream->encode_type);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
...
static void* venc_thread(void *p)
{
    ...
    memset(VencFd, 0xff, sizeof(VencFd));
    ...
    for (i = 0; i < sizeof(g_av_args->venc_chn)/sizeof(g_av_args->venc_chn[0]); i++)
    {
        if (g_av_args->venc_chn[i] != -1)
        {
            VencFd[i] = HI_MPI_VENC_GetFd(g_av_args->venc_chn[i]);
    ...
    {
        FD_ZERO(&read_fds);
        for (i = 0; i < sizeof(VencFd)/sizeof(VencFd[0]); i++)
        {
            if (VencFd[i] != -1)
            {
        ...
        for (i = 0; i < sizeof(VencFd)/sizeof(VencFd[0]); i++)
        {
            if (VencFd[i] != -1 && g_av_args->venc_chn[i] != -1 && FD_ISSET(VencFd[i], &read_fds))
            {
                memset(&stStat, 0, sizeof(stStat));
                memset(&stStream, 0, sizeof(stStream));
                
                VENC_CHN VencChn = g_av_args->venc_chn[i];
                ...
                {
                    s32Ret = venc_one_pack(VencChn, &stStream, &stStat);
                    CHECK(s32Ret == HI_SUCCESS, NULL, "Error with %#x.\n", s32Ret);
                }
                else
                {
                    s32Ret = venc_multiple_pack(VencChn, &stStream, &stStat);
                    CHECK(s32Ret == HI_SUCCESS, NULL, "Error with %#x.\n", s32Ret);
                }
            }
        }
...
int sal_sys_init1(sal_video_s* video)
{
    ...
    memset(g_av_args->vpss_chn, 0xff, sizeof(g_av_args->vpss_chn));
    memset(g_av_args->venc_chn, 0xff, sizeof(g_av_args->venc_chn));
    memset(g_av_args->vpss_venc_chn, 0xff, sizeof(g_av_args->vpss_venc_chn));
    ...
    if (g_av_args->scale_enable && g_av_args->sensor_type == SONY_IMX327_MIPI_1080P_30FPS)
    {
        g_av_args->vi_width = 1920;
        g_av_args->vi_height = 1080;
        g_av_args->ext_chn_enable = 1;
    ...
    s32Ret = vi_start_all(ViDev, IspDev);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);

    s32Ret = vpss_start(ViDev, VpssGrp);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
    
    unsigned int idx_chn_vpss = 0;
    unsigned int idx_chn_venc = 0;
    unsigned int idx_chn_vpss_venc = 0;
    int chn_tmp = 0;
    ...
    {
        chn_tmp = i;
        s32Ret = venc_set_chn(VpssGrp, chn_tmp);
        CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
        
        g_av_args->vpss_chn[idx_chn_vpss] = chn_tmp;
        idx_chn_vpss++;
        
        if (chn_tmp == 0 && g_av_args->ext_chn_enable)
        {
            chn_tmp = g_av_args->ext_chn_number;
            s32Ret = venc_set_chn_ext(VpssGrp, i, chn_tmp);
            CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
    ...
    for (i = 4; i < (unsigned int)g_av_args->video_chn_num+4; i++)
    {
        ...
        s32Ret = venc_set_cfg(VpssGrp, i);
        CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
        
        s32Ret = venc_set_qp(i);
        CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
    }
    //bind vpss and venc channel
    for (i = 0, j = 0; i < sizeof(g_av_args->venc_chn)/sizeof(g_av_args->venc_chn[0]); i++, j++)
    {
        if (g_av_args->venc_chn[i] != -1)
        {
            chn_tmp = g_av_args->vpss_chn[j];
            if (chn_tmp == 0 && g_av_args->ext_chn_enable)
            {
                chn_tmp = g_av_args->vpss_chn[j+1];
                j++;
            }
            s32Ret = venc_bind_vpss(VpssGrp, g_av_args->venc_chn[i], chn_tmp);
            CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
            
            g_av_args->vpss_venc_chn[idx_chn_vpss_venc][0] = chn_tmp;
            g_av_args->vpss_venc_chn[idx_chn_vpss_venc][1] = g_av_args->venc_chn[i];
            idx_chn_vpss_venc++;
        }
    }
    ...
    {//debug
        unsigned int m = 0;
        for (m = 0; m < sizeof(g_av_args->vpss_chn)/sizeof(g_av_args->vpss_chn[0]); m++)
        {
            if (g_av_args->vpss_chn[m] != -1)
            {
                DBG("vpss_chn: %d\n", g_av_args->vpss_chn[m]);
            }
        }
        for (m = 0; m < sizeof(g_av_args->venc_chn)/sizeof(g_av_args->venc_chn[0]); m++)
        {
            if (g_av_args->venc_chn[m] != -1)
            {
                DBG("venc_chn: %d\n", g_av_args->venc_chn[m]);
            }
        }
        for (m = 0; m < sizeof(g_av_args->vpss_venc_chn)/sizeof(g_av_args->vpss_venc_chn[0]); m++)
        {
            if (g_av_args->vpss_venc_chn[m][0] != -1 && g_av_args->vpss_venc_chn[m][1] != -1)
            {
                DBG("vpss_chn: %d bind venc_chn: %d\n", g_av_args->vpss_venc_chn[m][0], g_av_args->vpss_venc_chn[m][1]);
            }
        }
    }
...
int sal_sys_exit1(void)
{
    ...
    VENC_CHN VencChn = 0;
    int m = 0;
    for (m = sizeof(g_av_args->vpss_venc_chn)/sizeof(g_av_args->vpss_venc_chn[0]) - 1; m >= 0; m--)
    {
        if (g_av_args->vpss_venc_chn[m][0] != -1 && g_av_args->vpss_venc_chn[m][1] != -1)
        {
            VpssChn = g_av_args->vpss_venc_chn[m][0];
            VencChn = g_av_args->vpss_venc_chn[m][1];
            s32Ret = venc_unbind_vpss(VencChn, VpssGrp, VpssChn);
            CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
            DBG("m[%d] venc[%d] unbind vpss[%d] done.\n", m, VencChn, VpssChn);
        }
    }
    
    for (m = sizeof(g_av_args->venc_chn)/sizeof(g_av_args->venc_chn[0]) - 1; m >= 0; m--)
    {
        if (g_av_args->venc_chn[m] != -1)
        {
            VencChn = g_av_args->venc_chn[m];
    ...
    for (m = sizeof(g_av_args->vpss_chn)/sizeof(g_av_args->vpss_chn[0]) - 1; m >= 0; m--)
    {
        if (g_av_args->vpss_chn[m] != -1)
        {
            VpssChn = g_av_args->vpss_chn[m];
    ...
    //vpss unbind
    s32Ret = vi_unbind_vpss(ViDev, VpssGrp);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
    DBG("UnBindVpss done.\n");
    ...
    //vi stop
    s32Ret = vi_stop_isp(ViDev, IspDev);
    CHECK(s32Ret == HI_SUCCESS, HI_FAILURE, "Error with %#x.\n", s32Ret);
    DBG("StopVi done.\n");
