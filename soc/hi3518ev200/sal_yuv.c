...
static int gbuffer_offset = NULL;
static int gbuffer_len = 0;
...
int sample_yuv_dump(VIDEO_FRAME_S* pVBuf, FILE* pfd)
{
    unsigned int w, h;
    char* pVBufVirt_Y;
    char* pVBufVirt_C;
    ...
    for (h = 0; h < pVBuf->u32Height; h++)
    {
        ...
        CHECK(gbuffer_len-gbuffer_offset > pVBuf->u32Width, -1, "buffer is too small\n");
        memcpy(gbuffer+gbuffer_offset, pMemContent, pVBuf->u32Width);
        gbuffer_offset += pVBuf->u32Width;
    }
...
HI_S32 vpss_chn_dump_main(int argc, const char* argv[])
{
    HI_U32 u32FrmCnt = 1;
...
int sal_yuv_get(int stream, char* buffer, int len)
{
    ...
    const char* chn = (stream == 0) ? "0" : "1";
    int argc = 4;
    const char* argv[] = {"null", "0", chn, "1"};
    vpss_chn_dump_main(argc, argv);
    DBG("capture yuv sp420 done.[%d bytes]\n", gbuffer_offset);

    return gbuffer_offset;
}
